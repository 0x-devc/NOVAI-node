...
use tx_generator::throttle::{
    next_level, Throttle, LOOSEN_BELOW, MAX_LEVEL, MIN_SAMPLES, TIGHTEN_ABOVE,
};
...
fn an_rpc_error_is_not_progress() {
    assert!(!poll_is_progress(&Err("connection refused".into()), Some(100)));
}
...
fn only_a_rising_height_counts_as_progress() {
    ...
    assert!(!poll_is_progress(&Ok(Some(99)), Some(100)), "went backwards");
}
