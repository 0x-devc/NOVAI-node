...
/// Version byte for AiEntity encoding.
pub const AI_ENTITY_V1: u8 = 0x01;

/// Encoded size of AiEntity v1 in bytes.
...
pub fn decode_ai_entity_v1(input: &[u8]) -> Result<AiEntity, AiEntityCodecError> {
    ...
    cursor += 1;
    if version != AI_ENTITY_V1 {
        return Err(AiEntityCodecError::InvalidVersion(version));
    }
...
mod tests {
    ...
    fn roundtrip_works() {
        let entity = test_entity();
        let encoded = encode_ai_entity_v1(&entity);
        let decoded = decode_ai_entity_v1(&encoded).unwrap();
    ...
    fn encoding_is_deterministic() {
        let entity = test_entity();
        let encoded1 = encode_ai_entity_v1(&entity);
        let encoded2 = encode_ai_entity_v1(&entity);
    ...
    fn decode_rejects_short_buffer() {
        ...
        let result = decode_ai_entity_v1(&short);
        assert_eq!(result, Err(AiEntityCodecError::BufferTooShort));
    }

    #[test]
    fn decode_rejects_invalid_version() {
        let mut encoded = encode_ai_entity_v1(&test_entity());
        encoded[0] = 0xFF; // Invalid version
        let result = decode_ai_entity_v1(&encoded);
