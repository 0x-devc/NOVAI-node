...
use novai_p2p::{NetworkMessage, PeerManager, connect_to_peer, read_wire_message, start_listener};
...
pub struct ConsensusNode {
    ...
    pub qc_broadcasted: Arc<Mutex<HashSet<(u64, u64, [u8; 32])>>>,
}
...
impl ConsensusNode {
    ...
    pub fn are_we_leader(&self) -> bool {
        ...
        match ConsensusState::compute_leader_for_view(state.height, state.round, &self.validator_set) {
    ...
    pub fn handle_proposal(&self, signed_proposal: SignedProposal) -> Result<(), String> {
       ...
       // 1. Check proposer is expected leader for this height/round
        ...
        let expected_leader = ConsensusState::compute_leader_for_view(view_height, block.round, &self.validator_set)
        ...
        let proposer_pubkey = self.validator_pubkeys
            .get(&signed_proposal.proposer)
            .ok_or_else(|| format!("Proposer {:?} not in validator set", &signed_proposal.proposer[..4]))?;

        let unsigned_bytes = novai_consensus_types::codec::encode_proposal_v1_unsigned(&signed_proposal.proposal)
            .map_err(|e| format!("Encode proposal failed: {:?}", e))?;

        if !novai_crypto::verify_bytes(proposer_pubkey, &unsigned_bytes, &signed_proposal.signature) {
            return Err(format!("Invalid proposal signature from {:?}", &signed_proposal.proposer[..4]));
        }
        ...
        if block.height == 1 {
            ...
            if justify_qc.votes.len() < quorum {
                return Err(format!(
                    ...
                    block.height, justify_qc.votes.len(), quorum
                ));
            }
        }
        ...
        state.verify_block(block, &*db)
            .map_err(|e| format!("Block verification failed: {:?}", e))?;

        // 5. Create vote
        let vote = state.create_vote(block, &self.signing_key)
    ...
    pub fn handle_vote(&self, vote: Vote) -> Result<(), String> {
        ...
        let leader_for_vote = {
            ...
            let leader_idx = ((proposal_state_height + vote.round) as usize) % self.validator_set.len();
            self.validator_set[leader_idx] == self.our_address
        };
        
        println!("DEBUG: Vote h={} r={}. Leader for proposal_height={}? {}", 
                 vote.height, vote.round, vote.height.saturating_sub(1), leader_for_vote);

        if !leader_for_vote {
            println!("DEBUG: Not leader for height {}, skipping QC formation", vote.height);
            return Ok(());
        }

        println!("DEBUG: Attempting QC formation for block_hash={:?}", &vote.block_hash[..8]);
