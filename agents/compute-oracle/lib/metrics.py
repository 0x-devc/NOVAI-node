...
- /metrics renders all DECLARED metrics in stable (sorted) order even
  ...
  series.
- novai_compute_oracle_uptime_seconds is computed at render time from the
...
LOG = logging.getLogger("compute_oracle.metrics")
...
class MetricsRegistry:
    ...
    for the locked metric set; reason="..." on the failure counters).
    """
    ...
    def render(self) -> str:
        ...
        for name in sorted(gauge_help):
            ...
            if name == "novai_compute_oracle_uptime_seconds":
...
def build_compute_oracle_registry(start_time_monotonic: float) -> MetricsRegistry:
    """Declare every metric the compute oracle emits.

    Reason labels match the values produced by lib.chain.map_submit_error
    and the lib.gpu_source exception classes. The dry-run counter records
    signals that were constructed and logged but deliberately not submitted.
    """
    reg = MetricsRegistry(start_time_monotonic)
    reg.declare_counter(
        "novai_compute_oracle_price_fetch_success_total",
        "Number of successful GPU pricing fetches.",
    )
    reg.declare_counter(
        "novai_compute_oracle_price_fetch_failure_total",
        "Number of failed GPU pricing fetches, by reason.",
        label_name="reason",
        label_values=(
            "rate_limit",
            "server_error",
    ...
    reg.declare_counter(
        "novai_compute_oracle_submission_success_total",
        "Number of signals accepted by the chain (live mode only).",
    )
    reg.declare_counter(
        "novai_compute_oracle_submission_failure_total",
        "Number of submissions rejected or failed, by reason.",
        label_name="reason",
        label_values=(
            "rpc_unreachable",
            "rpc_error",
    ...
    reg.declare_counter(
        "novai_compute_oracle_dry_run_constructed_total",
        "Number of signals constructed and logged in DRY_RUN, by kind (not submitted).",
        label_name="kind",
        label_values=("oracle_anchor", "reputation_update"),
    )
    reg.declare_counter(
        "novai_compute_oracle_faucet_attempts_total",
        "Number of re-faucet attempts made from the runtime loop, by outcome (live only).",
        label_name="result",
        label_values=(
            "success",
            "rate_limited",
    ...
    reg.declare_counter(
        "novai_compute_oracle_credit_attempts_total",
        "Number of CreditAiEntity attempts made from the runtime loop, by outcome (live only).",
        label_name="result",
        label_values=(
            "success",
            "nonce_mismatch",
    ...
    reg.declare_gauge(
        "novai_compute_oracle_last_price_usd_per_hour",
        "Most recent observed GPU price (median on-demand USD per hour for the model).",
    )
    reg.declare_gauge(
        "novai_compute_oracle_last_sample_size",
        "Number of marketplace offers that fed the most recent observation.",
    )
    reg.declare_gauge(
        "novai_compute_oracle_entity_balance",
        "Last observed entity.economic_balance for the oracle entity (live mode).",
    )
    reg.declare_gauge(
        "novai_compute_oracle_account_balance",
        "Last observed account.balance for the oracle funder address (live mode).",
    )
    reg.declare_gauge(
        "novai_compute_oracle_last_submission_height",
        "Chain head height at the moment of the last successful submission (live mode).",
    )
    reg.declare_gauge(
        "novai_compute_oracle_last_loop_completed_timestamp",
        "Unix epoch seconds when the last loop iteration finished (success or fail).",
    )
    reg.declare_gauge(
        "novai_compute_oracle_uptime_seconds",
